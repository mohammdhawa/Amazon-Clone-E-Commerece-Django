...
def mydebug(request):
    ...
    # Relation
    # data = Product.objects.filter(brand__id=5) # here when I write brand__ so now I'm in brand object(table)
    # data = Product.objects.filter(brand__id__lt=5) # here we want all products that their brand (brand id less than 5)
...
def add_review(request, slug):
    ...
    review = request.POST['review'] # or request.GET['review] if the method is get or request.GET.get('review') or request.POST.get('review')
